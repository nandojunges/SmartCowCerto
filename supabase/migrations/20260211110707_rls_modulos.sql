...
-- fazendas => módulo ajustes
alter table public.fazendas enable row level security;

drop policy if exists fazendas_select_modulo on public.fazendas;
create policy fazendas_select_modulo
  on public.fazendas
  for select
  using (
    public.fazendas.owner_user_id = auth.uid()
    or (
      exists (
        select 1
        from public.fazenda_acessos fa
        where fa.fazenda_id = public.fazendas.id
          and fa.user_id = auth.uid()
          and upper(coalesce(fa.status, '')) = 'ATIVO'
      )
      and exists (
        select 1
        from public.fazenda_permissoes fp
        where fp.fazenda_id = public.fazendas.id
          and fp.user_id = auth.uid()
          and fp.modulo = 'ajustes'
          and fp.pode_ver = true
      )
...
drop policy if exists fazendas_insert_modulo on public.fazendas;
create policy fazendas_insert_modulo
  on public.fazendas
  for insert
  with check (
    public.fazendas.owner_user_id = auth.uid()
...
drop policy if exists fazendas_update_modulo on public.fazendas;
create policy fazendas_update_modulo
  on public.fazendas
  for update
  using (
    public.fazendas.owner_user_id = auth.uid()
...
drop policy if exists fazendas_delete_modulo on public.fazendas;
create policy fazendas_delete_modulo
  on public.fazendas
  for delete
  using (
    public.fazendas.owner_user_id = auth.uid()
...
-- profiles => módulo ajustes (owner por padrão)
alter table public.profiles enable row level security;

drop policy if exists profiles_select_modulo on public.profiles;
create policy profiles_select_modulo
  on public.profiles
  for select
  using (public.profiles.id = auth.uid());

drop policy if exists profiles_insert_modulo on public.profiles;
create policy profiles_insert_modulo
  on public.profiles
  for insert
  with check (public.profiles.id = auth.uid());

drop policy if exists profiles_update_modulo on public.profiles;
create policy profiles_update_modulo
  on public.profiles
  for update
  using (public.profiles.id = auth.uid())
  with check (public.profiles.id = auth.uid());

drop policy if exists profiles_delete_modulo on public.profiles;
create policy profiles_delete_modulo
  on public.profiles
  for delete
  using (public.profiles.id = auth.uid());
