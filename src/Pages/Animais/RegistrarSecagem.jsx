...
export default function RegistrarSecagem(props) {
  ...
  const [mastiteTeste, setMastiteTeste] = useState(null); // Sim/Não
  const [tratamentoSecagem, setTratamentoSecagem] = useState(null); // Nenhum / Selante / Antibiotic+Selante
  ...
  useEffect(() => {
    ...
    // tenta puxar parto previsto do animal (se existir em qualquer chave comum)
    const pp =
      animalSafe?.parto_previsto ??
      animalSafe?.data_parto_previsto ??
      animalSafe?.data_prev_parto ??
      animalSafe?.previsao_parto ??
      animalSafe?.partoPrevisto ??
      null;

    if (pp) setDataPartoPrevisto(isoToDDMMYYYY(pp));
  ...
  const salvar = async () => {
    ...
    const payload = {
      ...
      meta: {
        ...
        mastite_teste: mastiteTeste?.value ?? null,
    ...
    <div style={estilos.overlay} onClick={(e) => e.target === e.currentTarget && onClose?.()}>
      <div style={estilos.modal}>
        ...
        <div style={estilos.corpo}>
          ...
          <div style={estilos.linha2}>
            <div style={estilos.campo}>
              <label style={estilos.label}>Teste de mastite ao secar</label>
              <Select
                options={opcoesMastite}
                value={mastiteTeste}
                onChange={setMastiteTeste}
                styles={selectStyles}
